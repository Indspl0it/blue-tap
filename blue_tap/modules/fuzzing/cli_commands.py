...
from blue_tap.framework.envelopes.fuzz import build_fuzz_operation_result, make_fuzz_run_id
...
def _build_dashboard(
    *,
    ...
    if paused:
        title_text = f"[bold {YELLOW}]Blue-Tap Fuzzer -- PAUSED[/bold {YELLOW}]"
        border = YELLOW
    else:
        title_text = f"[bold {CYAN}]Blue-Tap Fuzzer -- Campaign Dashboard[/bold {CYAN}]"
        border = CYAN
...
def _campaign_command(fuzz_group):
    ...
    def campaign(
        ...
        Examples:
          ...
          blue-tap fuzz campaign -p sdp -p rfcomm --duration 30m
...
def _crash_commands(fuzz_group):
    ...
    def crashes_list(session_name, protocol, severity):
        ...
        Examples:
          ...
          blue-tap fuzz crashes list --protocol sdp
          blue-tap fuzz crashes list --severity CRITICAL
          blue-tap fuzz crashes list -s my_session
        """
    ...
    def crashes_show(crash_id, session_name):
        ...
        Examples:
          ...
          blue-tap fuzz crashes show 3 -s my_session
        """
        session_dir = _resolve_session_dir(session_name)
        if not session_dir:
    ...
    def crashes_replay(crash_id, session_name, capture):
        ...
        # Create transport
        try:
            from blue_tap.modules.fuzzing.transport import L2CAPTransport, RFCOMMTransport, BLETransport, RawACLTransport
            ttype = spec["type"]
            if ttype == "l2cap":
