...
Every bt-tap command auto-logs its structured output to the active session.
The report command then reads everything from the session directory.

Usage:
    bt-tap -s my_assessment scan classic        # logs scan results
    bt-tap -s my_assessment vulnscan AA:BB:...   # logs vuln findings
    bt-tap -s my_assessment report               # auto-collects everything
